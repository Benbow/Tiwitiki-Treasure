...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    private void InitPageSelection() {
        ...
        if (_showPageSelection) {
            ...
            for (int i = 0; i < _pageCount; i++)
            {
                ...
                selectIcon.transform.SetParent(pageSelectionIcons);
                selectIcon.transform.localScale = new Vector3(1, 1, 1);
                Image imgAdd = selectIcon.AddComponent<Image>();

                if (_pageCount % 2 == 0)
                {
                    if (i < (float)_pageCount / 2f)
                        selectIcon.transform.localPosition = new Vector3((_pageCount / 2 - i) * (-Constant._BetweenWorldSelectionIconSize) + Constant._BetweenWorldSelectionIconSize/2f, 0, 0);
                    else
                        selectIcon.transform.localPosition = new Vector3((-_pageCount / 2 + i) * (Constant._BetweenWorldSelectionIconSize) + Constant._BetweenWorldSelectionIconSize/2f, 0, 0);
                }
                else
                {
                    if (i < (float)_pageCount / 2f)
                        selectIcon.transform.localPosition = new Vector3((_pageCount / 2f - i) * (-Constant._BetweenWorldSelectionIconSize) + Constant._BetweenWorldSelectionIconSize / 2f, 0, 0);
                    else
                        selectIcon.transform.localPosition = new Vector3((-_pageCount / 2f + i) * (Constant._BetweenWorldSelectionIconSize) + Constant._BetweenWorldSelectionIconSize / 2f, 0, 0);
                }

                imgAdd.sprite = unselectedPage;
                imgAdd.SetNativeSize();
                selectIcon.transform.localScale *= 0.5f;
    ...
    private void SetPageSelection(int aPageIndex) {
        ...
        LocalManager.instance.ChangeWorld(aPageIndex + 1);
    }
